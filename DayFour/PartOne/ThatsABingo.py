...
def check_winner(current_score_matrix):
    win = False
    current_matrix_id = 0
    for i in range(current_score_matrix.shape[0]):
        for j in range(current_score_matrix.shape[1]):
            if np.all(current_score_matrix[i][j] == 1):
                ...
                print()
                print('Row with the winning numbers: ', j)
    ...
    for i in range(current_score_matrix.shape[0]):
        for j in range(current_score_matrix.shape[1]):
            if np.all(current_score_matrix[i][:, j] == 1):
                ...
                print(current_score_matrix[i])
                print()
                print('Column with the winning numbers: ', j)
    ...
    for matrix_id, matrix in enumerate(bingo_card_matrix_values):
        for line_id, line in enumerate(matrix):
            for number_id, number in enumerate(line):
                ...
                winner, winning_matrix_id = check_winner(bingo_card_matrix_guesses)
                winning_number = guess

                # If a winner was found , break out of the loop and return the winning card
                if winner:
                    break
...
print(bingo_card_matrix_values[winning_matrix_id])
